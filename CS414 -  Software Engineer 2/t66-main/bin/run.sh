...
export SERVER_PORT=$((0+${CS414_SERVER_BASE:-41400}+${CS414_TEAM:-0}))
export CLIENT_PORT=$((0+${CS414_CLIENT_BASE:-44100}+${CS414_TEAM:-0}))
...
function get_default_prefix {
  if [[ -n "$NFS_MOUNTED" ]]; then
    export BUILD_DIRECTORY_PREFIX="/tmp/cs414-$(whoami)"
  else
...
echo "Building and Starting the Application in $([[ "$CS414_ENV" == "dev" ]] && echo 'DEVELOPMENT' || echo 'PRODUCTION' ) Mode."
