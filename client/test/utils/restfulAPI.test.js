...
import {VALID_CONFIG_RESPONSE, INVALID_REQUEST, VALID_DISTANCES_RESPONSE} from '../sharedMocks';
...
describe('restfulAPI', () => {
    ...
    test('jbahny: sendAPIRequest handles valid server response for distances', async () => {
    ...
    test('jbahny: sendAPIRequest response is not ok for distances', async () => {
        fetch.mockResponse(INVALID_REQUEST, { status: 404, ok: false, statusText: 'This is what we expect' });
        const response = await sendAPIRequest(JSON.parse(VALID_DISTANCES_RESPONSE), 'http://localhost:31400');
        expect(response).toBeNull();
        expect(LOG.error.mock.calls.length).toBeGreaterThanOrEqual(1);
    ...
    test('jbahny: sendAPIRequest response is rejected for distances', async () => {
        fetch.mockReject(new Error('Expected rejection'));
        const response = await sendAPIRequest(JSON.parse(VALID_DISTANCES_RESPONSE), 'http://localhost:31400');
        expect(response).toBeNull();
        expect(LOG.error.mock.calls.length).toBeGreaterThanOrEqual(1);
    ...
    test('jbahny: sendAPIRequest schema not implemented for distances', async () => {
        fetch.mockResponse(VALID_DISTANCES_RESPONSE);
        expect(sendAPIRequest({ requestType: 'notValid' })).rejects.toThrow();
    });

    test('jbahny: sendAPIRequest gives null response for invalid request for distances', async () => {
        fetch.mockResponse(INVALID_REQUEST);
        const response = await sendAPIRequest(JSON.parse(VALID_DISTANCES_RESPONSE), 'http://localhost:31400');
        expect(response).toBeNull();
        expect(LOG.error.mock.calls.length).toBeGreaterThanOrEqual(1);
