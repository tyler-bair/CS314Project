export function latLngToText(latLng, precision = 2, type = "NONE") {
	const lat = latLng?.lat ?? latLng?.latitude;
	const lng = latLng?.lng ?? latLng?.longitude;
	switch(type){
		case "SVG":
			return latLng && lat !== undefined && lng !== undefined ? `${lng.toFixed(precision)},${lat.toFixed(precision)} ` : "";
		case "KML": 
			return latLng && lat !== undefined && lng!== undefined ? `${lng.toFixed(precision)},${lat.toFixed(precision)},0\n` : "";
		default :
