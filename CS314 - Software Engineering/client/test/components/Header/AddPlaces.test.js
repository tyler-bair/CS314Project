...
describe('AddPlace', () => {
	...
	test('base: handles string input', async () => {
		const stringInput = screen.getByTestId('string-input');
		user.type(stringInput, stringInputValues.valid);

		await waitFor(() => {
			expect(stringInput.value).toEqual(stringInputValues.valid);
		});
	})
});
